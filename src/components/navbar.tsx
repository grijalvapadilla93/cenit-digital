    ...
    <>
      <nav className="fixed top-0 w-full z-50 bg-[#131313]/60 backdrop-blur-xl">
        <div className="flex justify-between items-center px-4 md:px-8 py-4 max-w-screen-2xl mx-auto">
          ...
          >
            <span
              className={`block w-6 h-[2px] bg-purple-400 transition-all duration-300 ${
      ...
      {/* Mobile menu */}
      <div
        className={`fixed inset-0 z-40 bg-[#0a0a0a]/95 backdrop-blur-xl flex flex-col items-center justify-center gap-6 transition-opacity duration-300 md:hidden ${
      ...
      >
        ...
        <div className="flex items-center gap-4 mt-8">
          <button
            onClick={() => { toggleLang(); }}
            className="px-4 py-2 rounded-full border border-white/20 text-sm font-[family-name:var(--font-space-grotesk)] font-bold uppercase tracking-widest text-white/70 hover:text-white transition-all btn-press"
          >
            {lang === "en" ? "ES" : "EN"}
          </button>
          <Button
